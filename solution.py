        ...
        try:
            ...
            with pd.read_csv(file, chunksize=size) as reader:
                for chunk in reader:
                    ...
                    # write chunk to stdout, if header we choose write mode to overwrite any previous info in stdout file
                    if put_head:
                        df_main.to_csv(sys.stdout, index=False, mode='w', header=put_head, lineterminator='\n')
                    else:
                        df_main.to_csv(sys.stdout, index=False, mode='a', header=put_head, lineterminator='\n')
